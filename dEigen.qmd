...
\end{equation} 
Combining \eqref{eq-offh} and \eqref{eq-diagh} shows that for the eigenvector corresponding with $\lambda_s$ we have\footnote{If using it makes a formula more clear then symbol $\cdot$ is used for multiplication with a scalar.}
\begin{equation}
...
\begin{equation}
\mathcal{D}_{st}\lambda_i=2x_i'(\mathcal{D}_sA-\lambda_s\mathcal{D}_sB)\mathcal{D}_tx_i+
x_i'(D_{st}A-\lambda_s\mathcal{D}_{st}B)x_i-x_i'(\mathcal{D_s}B)x_i)\cdot\mathcal{D}_t\lambda_i.\label{eq-seclbd}
\end{equation} 
