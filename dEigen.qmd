...
The case in which the parameters $\theta$ are coefficients of linear combinations of matrices is
important in practice. Suppose we have $p+q$ matrices $A_s$ and $B_s$, such that
$A_s=0$ if $p+1\leq s\leq p+q$ and $B_s=0$ if $1\leq s\leq p$. There are also "intercepts"
$A_0$ and $B_0$ which may or may not be zero. Thus
