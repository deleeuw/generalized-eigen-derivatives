...
and each of them occurs in the data with frequency $\pi_s$ (in most MCA applications
...
derivatives of the scaled eigenvectors slightly more complicated.

The second derivatives are of course also compouted by gevNonlinear(). The Delta Method
tells us they can be used to give estimates of the bias, using
$$
N\cdot\text{BIAS}(\lambda_i)=\frac12\text{tr}\ \mathcal{D}^2\lambda_i(\Pi-\pi\pi')=\sum_{s=1}^p\pi_s\mathcal{D}_{ss}\lambda_i,
$$
...
```
We see that the larger eigenvalues are overestimated and the smaller ones are underestimated.
