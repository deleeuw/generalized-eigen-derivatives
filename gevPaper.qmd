...
x_i'(D_{st}A-\lambda_i\mathcal{D}_{st}B)x_i\\-x_i'(\mathcal{D}_tB)x_i\cdot x_i'(\mathcal{D}_sA-\lambda_i\mathcal{D}_sB)x_i-x_i'(\mathcal{D_s}B)x_i\cdot x_s'(\mathcal{D}_tA-\lambda_i\mathcal{D}_tB)x_s.\label{eq-seclbdmat}
\end{multline} 
...
Start with 
$$
\mathcal{D}_tW_i=X(\Lambda-\lambda_iI)^+(D_tX)'+(D_tX)(\Lambda-\lambda_iI)^+X'-XV_tX',
$$
where for $i\neq j$
$$
\{V_t\}_{ij}:=\frac{\mathcal{D}_t\lambda_j-\mathcal{D}_t\lambda_i}{(\lambda_j-\lambda_i)^2},
$$
and $\{V_t\}_{ii}:=0$. 
...
$q$ known symmetric matrices $B_s$. It is convenient to pretend there are  $p+q$ matrices $A_s$ and $B_s$, such that $A_s=0$ if $p+1\leq s\leq p+q$ and $B_s=0$ if $1\leq s\leq p$. There may also be "intercepts"
$A_0$ and $B_0$. Thus
\begin{subequations}
\begin{align}
...
If $A()$ and $B()$ are function of the same parameter vector $\Theta$, then 
there is no need to distinguish the separate cases of the disjoint
section. So we have

\begin{subequations}
\begin{align}
A(\theta)&=A_0+\sum_{s=1}^p\theta_s A_s,\\
B(\theta)&=B_0+\sum_{s=1}^p\theta_s B_s.
\end{align}
\end{subequations}
