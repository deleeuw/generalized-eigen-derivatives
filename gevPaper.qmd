...
f(1)&=\mathcal{D}f(1)=0,\\
\mathcal{D}^2f''(1)&=1,\\
\mathcal{D}f(\theta)&<0\text{ for }0<\theta<1,\\
\mathcal{D}f(\theta)&>0\text{ for }\theta>1.
\end{align}
Note that this implies that $f$ is unimodal and has a single minimum equal to zero at one. Also
$$
...
As Swain points out, both maximum likelhood estimation and generalized least squares estimation
(@joreskog_goldberger_72) are special cases. For maximum likelihodd
$$
...
$$
But the .. also covers other cases, such as
$$
f_3(\theta)=\frac12(\log\theta)^2.
$$
$$
f_4(\theta)=f_2(1/\theta)=\frac12\frac{(\theta-1)^2}{\theta^2}.
$$
In addition if $f$ satisfies ... then so does 
