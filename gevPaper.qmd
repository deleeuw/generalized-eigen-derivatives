...
to provide a basis for minimization by
Newton's method of some of these loss functions.

### Least Squares

In the Least Squares Factor Analysis (LSFA) of a covariance matrix $C$ of order $n$ we minimize
$$
\sigma(U,D)=\text{tr}\ (C-UU'-D)^2
$$
over the $n\times p$ matrices $U$ and the diagonal matrices $D$ of order $n$. An LSFA is called
proper if in addition we require $0\lesssim D\lesssim C$ (in the Loewner sense).

@thomson_34, @harman_jones_66, Zegers Ten Berge

If $C\gtrsim D$ then
$$
\sigma_\star(D)=\min_U\sigma(U,D)=\sum_{i=p+1}^n\lambda_i^2(C-D).
$$
$\mathcal{D}_iA=-e_ie_i'$
$$
\mathcal{D}_i\sigma_\star=-2\sum_{\nu=p+1}^n\lambda_\nu^{\ }x_{i\nu}^2
$$

$$
\mathcal{D}_{ij}\sigma_\star=2\sum_{\nu=p+1}^nx_{i\nu}^2x_{j\nu}^2-4\sum_{\nu=p+1}^n\sum_{\eta\neq\nu}^n\frac{\lambda_\nu}{\lambda_\eta-\lambda_\nu}x_{i\nu}x_{j\nu}x_{i\eta}x_{j\eta}
$$
Derflinger joreskog van thillo

The file lsfa.R in the github repository has functions to compute the first and second derivatives,
with a numDeriv checker.

### The Swain Family

In an important paper @swain_75 derived a class of factor analytic procedures which produce estimates 
asymptotically equivalent to the classical Lawley-Joreskog maximum likelihood estimates. Swain also
...
Suppose $f$ is a twice-differentiable univariate function defined on the positive real axis with the properties
\begin{subequations}
\begin{align}
...
\end{align}
\end{subequations}
Note that this implies that $f$ is unimodal and has a single minimum equal to zero at one. Also, by l'Hôpital.
\begin{equation}
...
\sigma(D):=\sum_{\nu=p+1}^n f(\lambda_\nu(A)),
\end{equation}
with $\lambda$ the eigenvalues of $A:=S^{-\frac12}DS^{-\frac12}$.
Both maximum likelhood estimation, generalized least squares estimation
(@joreskog_goldberger_72), and the "geodesic distance" (@james_73) are special cases. 
For maximum likelihood
\begin{subequations}
\begin{equation}
f_1(\theta):=1/\theta+\log\theta-1
\end{equation}
and for GLS with loss $\frac12\text{tr}\ S^{-1}(\Sigma-S)S^{-1}(\Sigma-S)$ we have
\begin{equation}
f_2(\theta):=\frac12(\theta-1)^2
\end{equation}
But \eqref{eq-f1}-\eqref{eq-f4} also covers other cases, such as the "geodetic distance" between covariance matrices, which has
\begin{equation}
f_3(\theta):=\frac12(\log\theta)^2,
\end{equation}
and the GLS variant with loss $\frac12\text{tr}\ \Sigma^{-1}(\Sigma-S)\Sigma^{-1}(\Sigma-S)$ 
\begin{equation}
f_4(\theta):=\frac12\frac{(\theta-1)^2}{\theta^2}.
\end{equation}
\end{subequations}
In addition if $f(\theta)$ satisfies \eqref{eq-f1}-\eqref{eq-f4} for all $\theta$ then so does 
$f(\theta^{-1})$. In fact $f_4(\theta)=f_2(\theta^{-1})$.
...
for the loss functions ..... There is also a numDeriv checker. The functions are programmed in such a way that they can easily be extended to other loss functions satisfying ...
...
that some of its elements are restricted to be equal to zero.
