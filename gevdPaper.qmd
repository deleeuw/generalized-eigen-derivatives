...
In Matrix Decomposition Factor Analysis (MDFA) we have to maximize the trace or nuclear norm, i.e. the sum of the singular values, of a matrix $XT$ over $T$. This is equivalent to
maximizing the sum of the square roots of the largest eigenvalues of 
$T'X'XT$. In MDFA the matrix $X$ is $n\times m$ and $T$ is $m\times p$, with $m<p$.
...
of its $m$ non-zero eigenvalues (which is equal to the trace norm). The matrix $T$ will 
be patterned, which means that some of its elements are fixed at known values, usually zero. I
In exploratory MDFA we typically have $T=\begin{bmatrix}U&D\end{bmatrix}$, with an $n\times(p-n)$
matrix $U$ and a diagonal matrix $D$. Thus the pattern is that the off-diagonal
elements of $D$ are zero. In confirmatory MDFA some elements of $U$ are fixed to known values.

If $A=T'CT$, with $C=X'X$ a constant matrix, and $T(\theta)=T_0+\sum_{s=1}^p\theta_s T_s$, we have
\begin{equation}
...
where $(T'CT)^{-\frac12}$ is the square root of the Moore-Penrose inverse. If the $T_s$ are indicators\footnote{Indicators have one element equal to one, with the other elements zero.}, we find
\begin{equation}
...
\end{equation}
where, from \eqref{eq-hessvalsimple},
\begin{equation}
\mathcal{D}_{st}\lambda_\nu=-2x_\nu'(T_s'CT+T'CT_s)(A-\lambda_\nu I)^+(T_t'CT+T'CT_t)x_\nu
\end{equation}
