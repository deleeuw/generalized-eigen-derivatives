...
$(1\ 0)$ and $1-\theta$
with eigenvector $(0\ 1)$. If $\theta=0$ both eigenvalues are $+1$, and any two-element
...
derivatives of the largest eigenvalue are $-1$ and $+1$, so the derivative does 
not exist.

This shows one way to deal with the problem. If $A()$ and $B()$ are differentiable
functions of $\theta$ then there are $n$ functions $\lambda_\nu()$ such that
...
It is just that $\lambda_\nu(\theta)$ may be the largest eignevalue for some $\theta$
...
more general derivatives, such as one-sided directional derivatives and subdifferentials.
This is the approach taken in convex analysis (@hirriart-urruty_ye_95, @lewis_overton_96). It mainly applies to eigenvalues and singular values in EVD and SVD problems with $B=I$, and not so much to the 
...
An Eigenvalue Decomposition (EVD) problem is of the form 
\begin{subequations}
\begin{align}
AX&=X\Lambda,\\
X'X&=I.
