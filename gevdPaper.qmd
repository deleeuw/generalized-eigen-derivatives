...
An Eigenvalue Decomposition (EVD) problem, thus without the 
...
System \eqref{eq-gsvdgevd} has $n+m$ solutions that can be expressed using the GSVD solutions
...
Suppose $A()$ and $B()$ are functions of proportions based on $N$ iid observations. The sample proportions are the parameters $\theta$ and their expected values are $\overline\theta$. It follows that $N^{\tfrac12}(\theta-\overline{\theta})$ is asymptotically normal with mean zero and variance $\overline{\Theta}-\overline{\theta}\overline{\theta}'$, where $\overline{\Theta}$ is a diagonal matrix with $\overline{\theta})$ on the diagonal.


### Correspondence Analysis

Correspondence analysis is a GSVD, with $F$ the contingency table, $G$ a diagonal matrix wwith row sums,
and $H$ a diagonal matrux with column sums. Thus $\mathcal{D}F$ is a unit matrix, and $\mathcal{D}G$
and $\mathcal{D}H$ are diagonal matrices with row and columns sums of $\mathcal{D}F$. All second derivatives
are zero.

The file gevdTemplateCA.R uses the 
