...
The generalized eigenvalue (GEV) problem for a pair $(A,B)$ of square symmetric matrices of order $n$ is to find the solution $(X,\Lambda)$ of the system
\begin{subequations}
\begin{align}
...
\begin{equation}
\mathcal{D}_sx_\nu=-\sum_{j\\eta\neq \nu}\frac{x_\eta'\{\mathcal{D}_sA-\lambda_\nu\mathcal{D}_sB\}x_\nu}{\lambda_\eta-\lambda_\nu}x_\eta-\tfrac12 x_\nu'(\mathcal{D}_sB)x_\nu\cdot x_\nu,\label{eq-secder1}
\end{equation}
which can also be written as
\begin{equation}
\mathcal{D}_sx_\nu=-\sum_{\eta\neq \nu}\left\{\frac{x_\eta x_\eta'}{\lambda_\eta-\lambda_\nu}\right\}(\mathcal{D}_sA-\lambda_\nu\mathcal{D}_sB)x_\nu-\tfrac12 (x_\nu'(\mathcal{D}_sB)x_\nu)x_\nu.\label{eq-secder2}
\end{equation}
...
result. First
\begin{multline}
\mathcal{D}_{st}x_\nu =-\sum_{\eta\neq\nu}\mathcal{D}_t\left\{\frac{x_j'\{\mathcal{D}_sA-\lambda_\nu \mathcal{D}_sB\}x_\nu }{\lambda_\eta-\lambda_\nu }\right\}x_\eta\\
...
There is a corresponding numDeriv based check function gevNonlinearNum(), which has the same single argument $\theta$ and gives the same output as gevNonlinear(). 
...
\end{align}
The template for a special case with $n=4$ and $p=3$ is gevdExampleOne.R in 
...
The template for a special case with $n=4$ and $p=6$ is gevdExampleTwo.R in 
the [github repository](https://www.github.com/deleeuw/generalized-eigen-derivatives).
...
&-(A-\lambda_\nu  I)^+(\mathcal{D}_{st}A)x_\nu \notag\\
&-(A-\lambda_\nu  I)^+(\mathcal{D}_{s}A)\mathcal{D}_tx_\nu .\label{eq-hessgensimple}
\end{align}
...
In the Simple Singular Value Decomposition (SSVD) we have both $G=I$ and $H=I$, and the corresponding derivatives are consequently zero, as are the matrices $U_s$ and $V_s$. This simplifies matters 
considerably
